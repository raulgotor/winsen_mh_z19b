...
#define MH_Z19B_MSG_START_VALUE_BYTE         (0)
#define MH_Z19B_MSG_CHECK_VALUE_BYTE         (8)

#define MH_Z19B_MSG_SET_COMMAND_BYTE         (2)
#define MH_Z19B_MSG_SET_PAYLOAD_START_BYTE   (3)
#define MH_Z19B_MSG_GET_PAYLOAD_START_BYTE   (2)

#define MH_Z19B_MSG_START_VALUE              (0xFF)
#define MH_Z19B_MSG_SENSOR_NUMBER            (0x01)
...
typedef enum {
        ...
        MH_Z19B_COMMAND_GAS_CONCENTRATION = 0x86,

        //! @brief Calibrate zero point
        MH_Z19B_COMMAND_CAL_ZERO_POINT = 0x87,

        //! @brief Calibrate span point
        MH_Z19B_COMMAND_CAL_SPAN_POINT = 0x88,

        //! @brief Set auto baseline correction on / off
        MH_Z19B_COMMAND_SET_ABC = 0x79,

        //! @brief
        MH_Z19B_COMMAND_SET_RANGE = 0x99,

        //! @brief Fence member
        MH_Z19B_COMMAND_COUNT
} mh_z19b_command_t;
...
uint8_t const m_tx_message_template[] = {
                MH_Z19B_MSG_START_VALUE,
                MH_Z19B_MSG_SENSOR_NUMBER,
                0x00,
                0x00,
                0x00,
                0x00,
...
static mh_z19b_error_t send_command(mh_z19b_command_t command,
...
static bool is_valid_command(mh_z19b_command_t const command);
...
static mh_z19b_xfer_func m_xfer_func = NULL;
 ...
 * @retval              MH_Z19B_ERROR_ALREADY_INITIALIZED
 *                                          Module is already initialized
 * @retval              MH_Z19B_ERROR_BAD_PARAMETER
 *                                          Parameter is null
 */
mh_z19b_error_t mh_z19b_init(mh_z19b_xfer_func const xfer_func)
{
        mh_z19b_error_t result = MH_Z19B_ERROR_SUCCESS;

        if (m_is_initialized) {
                result = MH_Z19B_ERROR_ALREADY_INITIALIZED;
 ...
 * @retval              MH_Z19B_ERROR_SUCCESS
 *                                          Everything went well
 * @retval              MH_Z19B_ERROR_NOT_INITIALIZED
 *                                          Module isn't initialized
 * @retval              MH_Z19B_ERROR_BAD_PARAMETER
 *                                          Parameter is null
 */
mh_z19b_error_t mh_z19b_get_gas_concentration(uint32_t * const p_concentration)
{
        ...
        if (!m_is_initialized) {
                result = MH_Z19B_ERROR_NOT_INITIALIZED;
        } else if (NULL == p_concentration) {
                result = MH_Z19B_ERROR_BAD_PARAMETER;
        } else {
                result = send_command(MH_Z19B_COMMAND_GAS_CONCENTRATION, NULL, 0);
        }
        ...
        if ((MH_Z19B_ERROR_SUCCESS == result) && (is_valid_message(rx_buffer))) {
                concentration =(rx_buffer[MH_Z19B_MSG_GET_PAYLOAD_START_BYTE] << 8);
                concentration |= (rx_buffer[MH_Z19B_MSG_GET_PAYLOAD_START_BYTE + 1]);
 ...
 */
mh_z19b_error_t mh_z19b_calibrate_zero_point(void)
{
        mh_z19b_error_t result;
        ...
        if (!m_is_initialized) {
                ...
                result = send_command(MH_Z19B_COMMAND_CAL_ZERO_POINT, NULL, 0);
        }
 ...
 * @retval              *                   Any other error
 */
mh_z19b_error_t mh_z19b_calibrate_span_point(uint16_t const span_point)
{
        ...
        if (!m_is_initialized) {
                ...
                result = send_command(MH_Z19B_COMMAND_CAL_SPAN_POINT,
                                      payload,
                                      payload_size);
        }
 ...
 * @retval              *                   Any other error
 */
mh_z19b_error_t mh_z19b_enable_abc(bool enabled)
{
        mh_z19b_error_t result;
        uint8_t payload;
        ...
        if (!m_is_initialized) {
                ...
                result = send_command(MH_Z19B_COMMAND_SET_ABC, &payload, 1);
        }
 ...
 * @retval              *                   Any other error
 */
mh_z19b_error_t mh_z19b_set_range(mh_z19b_range_t const range)
{
        ...
        if (!m_is_initialized) {
                ...
                result = send_command(MH_Z19B_COMMAND_SET_RANGE,
                                      payload,
                                      payload_size);
        }
                                   ...
                                   size_t const payload_size)
{
        ...
        if (!is_valid_command(command)) {
                ...
                message[MH_Z19B_MSG_SET_COMMAND_BYTE] = command;

                // Will be ignored if `payload_size` is 0
                memcpy(&message[MH_Z19B_MSG_SET_PAYLOAD_START_BYTE],
                       p_payload,
                       payload_size);

                check_val = calculate_check_value(message);

                message[MH_Z19B_MSG_CHECK_VALUE_BYTE] = check_val;
...
static uint8_t calculate_check_value(uint8_t const * const p_message)
{
        ...
        if (NULL != p_message) {
                for (i = 1; MH_Z19B_MSG_CHECK_VALUE_BYTE > i; ++i) {
...
static bool is_valid_message(uint8_t const * const p_message)
{
        uint8_t const start_val = p_message[MH_Z19B_MSG_START_VALUE_BYTE];
        uint8_t const check_val = p_message[MH_Z19B_MSG_CHECK_VALUE_BYTE];
        uint8_t const calculated_check_val = calculate_check_value(p_message);


        return ((check_val == calculated_check_val) &&
                (MH_Z19B_MSG_START_VALUE == start_val));
}

/*!
 * @brief Validate command
 *
 * Check whether the command is a valid `mh_z19b_command_t` command
 *
...
static bool is_valid_command(mh_z19b_command_t const command)
{
        ...
        case MH_Z19B_COMMAND_GAS_CONCENTRATION:
        case MH_Z19B_COMMAND_CAL_ZERO_POINT:
        case MH_Z19B_COMMAND_CAL_SPAN_POINT:
        case MH_Z19B_COMMAND_SET_ABC:
                success = true;
                break;
        default:
                success = false;
